      ...
      <div className="card p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          {canEdit ? (
            ...
            <span className={cn('text-xs font-medium px-3 py-1.5 rounded-full shrink-0', STATUS_COLORS[project.status])}>
              {STATUS_LABELS[project.status]}
            </span>
          )}
          </div>
        ...
        <div className="flex items-center gap-2 mt-4 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground ml-1">
            {myRole === 'owner' && <span className="flex items-center gap-1"><Crown className="w-3 h-3 text-amber-500" /> Dono</span>}
          </span>
          {canEdit && (
            <Link
              to={`/projetos/${project.id}/editar`}
              className="ml-auto flex items-center gap-1.5 text-xs text-muted-foreground hover:text-slate-700 transition-colors px-2 py-1 rounded-lg hover:bg-muted"
            >
